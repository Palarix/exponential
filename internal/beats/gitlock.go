...
// WithGitLock acquires an exclusive file lock on .beats/git.lock
...
func WithGitLock(fn func() error) error {
	lockPath := filepath.Join(".beats", "git.lock")
	...
	if !ok {
		return fmt.Errorf("another beats operation is using git — try again in a moment")
	}
	defer fl.Unlock()
