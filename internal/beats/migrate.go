...
import (
	...
	"github.com/palarix/beats/internal/sortorder"
...
func RunMigrations(currentVersion int) (int, error) {
	...
	if currentVersion < 2 {
		return 0, fmt.Errorf("data model v%d is not compatible with v%d; please run `beats init` to create a fresh database", currentVersion, version.DataModelVersion)
	}
...
func migrateV2ToV3() error {
	...
	for i, issue := range ordered {
		...
		evt := model.Event{
			...
			CreatedAt: timestamp,
			CreatedBy: "beats migrate <system>",
		}
