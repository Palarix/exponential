...
func TestBuildInbox_OnlyCreatorAndAssignee(t *testing.T) {
	// Issue created by bob; alice only commented and made a status update.
	// With the tighter filter (creator + assignee only), alice should NOT
	// see events on this issue because she is neither.
	events := []model.Event{
		...
		{ID: "x", Type: model.EventTypeComment, CreatedBy: me, CreatedAt: ts("2026-06-15T09:30:00Z"),
			Payload: model.CommentPayload{ID: "c1", Text: "I have thoughts"}},
	...
	if len(items) != 0 {
		t.Fatalf("expected 0 items (not creator or assignee), got %d: %+v", len(items), items)
	}
}
...
func TestFormatInboxItem(t *testing.T) {
	cases := []struct {
		...
		{
			name: "assigned to me",
			item: InboxItem{IssueID: "issue-b", Type: model.EventTypeUpdate, CreatedBy: carol,
				Payload: model.UpdatePayload{Assignee: strptr(me)}},
			want: "Carol QA assigned issue-b to you",
		},
		{
		...
		},
		{
			...
			want: "Bob Ops changed issue-a status to BLOCKED",
		},
		{
			name: "comment",
			item: InboxItem{IssueID: "issue-a", Type: model.EventTypeComment, CreatedBy: bob,
				Payload: model.CommentPayload{Text: "Looks good"}},
			want: `Bob Ops commented on issue-a "Looks good"`,
		},
		{
			name: "merge",
			item: InboxItem{IssueID: "issue-c", Type: model.EventTypeMerge, CreatedBy: bob,
				Payload: model.MergePayload{Strategy: "squash"}},
			want: "Bob Ops merged issue-c via squash",
		},
	}
...
func TestFormatInboxItem_MapPayload(t *testing.T) {
	...
	if got, want := FormatInboxItem(item, me), "Bob Ops merged issue-c via squash"; got != want {
