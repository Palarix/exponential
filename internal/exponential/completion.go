...
func CheckCompletionConfig() CompletionResult {
	...
	if configFile != "" {
		...
		if err == nil {
			...
			// "xpo completion <shell>" is the core indicator
			if strings.Contains(string(content), "xpo completion "+shell) {
				configured = true
			}
		}
	}
...
func GetCompletionInstallCmd(shell string) string {
	...
	if rcFile == "" {
		return fmt.Sprintf("# Could not determine config file, please see `xpo completion %s --help`", shell)
	}
	...
	switch shell {
	case "zsh":
		return fmt.Sprintf("echo 'source <(xpo completion zsh)' >> %s", displayRC)
	case "bash":
		return fmt.Sprintf("echo 'source <(xpo completion bash)' >> %s", displayRC)
	case "fish":
		return "xpo completion fish > ~/.config/fish/completions/xpo.fish"
	default:
		return fmt.Sprintf("# Please see `xpo completion %s --help`", shell)
	}
}
