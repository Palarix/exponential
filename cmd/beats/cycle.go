...
var cycleCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		if !cfg.Cycles.Enabled {
			fmt.Println("Cycles are not configured. Run 'beats cycle init' to set up.")
			os.Exit(1)
		}
...
var cycleHistoryCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		issues, err := client.ListIssues(beats.FilterOptions{All: true})
		if err != nil {
			fmt.Printf("Error: %v\n", err)
			os.Exit(1)
		}
...
func printCycleWithIssues(c config.Cycle, cfg *config.Config) {
	...
	client := beats.NewClient(cfg)
	issues, err := client.ListIssues(beats.FilterOptions{
		CycleID: c.ID,
		All:     true,
	})
	if err != nil {
