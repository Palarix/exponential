...
func runMerge(id string) {
	...
	if issue.BranchStats == nil {
		...
		fmt.Println("Create a branch with 'beats start', or check that the branch name contains the issue ID.")
		os.Exit(1)
	}

	if issue.BranchStats.Commits == 0 {
		fmt.Printf("Branch %s has no commits ahead of %s.\n", issue.BranchStats.Branch, beats.DefaultBranch())
		os.Exit(0)
	}
	...
	// Determine strategy
	var strategy beats.MergeStrategy
	switch {
	case mergeSquash:
...
func promptStrategy() beats.MergeStrategy {
	...
	case "2":
		return beats.MergeStrategyMerge
	case "3":
		return beats.MergeStrategyFF
	default:
		return beats.MergeStrategySquash
	}
}
