...
var loginCmd = &cobra.Command{
	...
	Short: "Authenticate against a remote xpo server",
	Long: `Authenticate against a remote xpo server using SSH key authentication.
...
If no URL is given, the remote URL from .xpo/config.yaml is used.
On first login, a minimal .xpo/config.yaml is created automatically.

Examples:
  xpo login https://exponential.example.com
  xpo login http://localhost:8080
  xpo login`,
...
func runLogin(cmd *cobra.Command, args []string) error {
	...
	if len(args) == 1 && config.IsLocalProjectConfig() && config.ReadRemoteURL() == "" {
		return fmt.Errorf(".xpo/config.yaml appears to be a local project config (has prefix/version) — run login from a client directory, not the server project")
	}
	...
	// 4. Save token to ~/.config/xpo/user.yaml
	if err := config.SaveServerCredential(serverURL, verifyResp.Token); err != nil {
		return fmt.Errorf("failed to save token: %w", err)
	}

	// 5. Write remote URL to .xpo/config.yaml (only when URL was explicitly provided)
	if len(args) == 1 {
...
func resolveServerURL(args []string) (string, error) {
	...
	if existing := config.ReadRemoteURL(); existing != "" {
		fmt.Printf("Using remote from .xpo/config.yaml: %s\n", existing)
		return existing, nil
	}
	return "", fmt.Errorf("no server URL provided and no remote configured in .xpo/config.yaml")
}
...
func writeProjectRemote(serverURL string) error {
	...
	if existing != "" {
		if existing != serverURL {
			fmt.Printf("Warning: .xpo/config.yaml already has remote URL %q (not overwriting)\n", existing)
		}
		return nil
	}
	...
	fmt.Println("Wrote remote URL to .xpo/config.yaml")
	return nil
}
