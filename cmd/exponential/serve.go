...
import (
	...
	"github.com/palarix/exponential/internal/mcpserver"
...
var serveCmd = &cobra.Command{
	Use:   "serve",
	Short: "Start a headless xpo server with auth",
	Long: `Starts a headless HTTP server exposing the xpo REST API and MCP
endpoint, both behind SSH key + JWT bearer token authentication.

The server reads .xpo/authorized_keys for user verification and
auto-generates a signing key at .xpo/server.key on first run.
...
func runServe(cmd *cobra.Command, args []string) error {
	...
	serverKeyPath := filepath.Join(xpoDir, "server.key")
	...
	authKeysPath := filepath.Join(xpoDir, "authorized_keys")
	...
	log.Printf("xpo serve listening on %s", listener.Addr())
