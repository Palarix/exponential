# Beats — Product Feature Map

> A snapshot of what beats does today, written for product/market-fit analysis and roadmap planning.
...
Beats is a **local-first, git-native project management tool**. It ships as a single binary that stores all data in a `.beats/` folder alongside your code. No accounts, no SaaS, no infrastructure — `beats init` and you're running.

It serves three interfaces from the same data:

- **CLI** — for developers who live in the terminal
- **Web UI** — a local board (`beats board`) for visual planning
...
- Accessible via `beats history`, the web UI activity timeline, and the MCP server
...
A React single-page app served locally by `beats board`. Views:
...
`beats_list`, `beats_show`, `beats_history`, `beats_add`, `beats_update`, `beats_comment`, `beats_link`
...
All data is an append-only JSONL event log (`.beats/issues.db`). Current state is a projection computed from replaying events. This gives:

- Complete audit trail by design
- Minimal merge conflicts (appending lines to a file)
- Portable — it's a text file in your repo

A local snapshot cache (`.beats/issues.snapshot.json`, gitignored) accelerates reads.

### Git-Native

- `.beats/` is committed alongside code — issues travel with the repo
- `beats start` auto-creates a branch from the issue ID and title
...
Layered config: built-in defaults → user `~/.config/beats/config.yaml` → project `.beats/config.yaml` → environment variables. Key settings:
...
- Branch creation on `beats start`
...
- `beats doctor` auto-detects and configures agent instruction files (CLAUDE.md, AGENTS.md, GEMINI.md)
- Duplicate detection on issue creation

### Multi-Instance

- Local registry tracks running `beats board` instances across projects
- Sidebar links between project boards
- Auto port selection to avoid conflicts

---

## What's NOT in Beats Today

Gaps and absent capabilities, for roadmap context:

- **No hosted/multi-user mode** — single-user, local only (epic `beats-075586` covers this)
...
- **No cross-repo issues** — each `.beats/` is scoped to one repository
