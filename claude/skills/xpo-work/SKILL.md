---
name: xpo-work
description: "Pick up a planned xpo issue and work through it following the full xpo discipline: discover, start, implement, test, document, and complete."
disable-model-invocation: true
---

# Work on a Exponential Issue

You are following the xpo-driven development workflow. This is a disciplined process — every step matters.
...
```bash
xpo add "Discovered issue title" --label "bug" --desc "What you found"
xpo link <new-id> <current-id> -t "relates_to"
```
...
- Never skip the `xpo start` step — the tracker must reflect what you're doing.
- Never mark `xpo done` without adding a summary comment first.
- Never mark `xpo done` without user approval of the changes.
