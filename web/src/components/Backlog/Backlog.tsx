...
export default function Backlog({
  ...
  // Initialize expanded groups: restore from localStorage or use defaults
  useEffect(() => {
      ...
      if (expandedGroups.has(status) && groupIssues.length > 0) {
        ...
        if (isAllTab) {
          ...
          const addTree = (
            ...
            const doneCount = allChildren.filter(
              (c) => c.status === "DONE",
            ).length;
            result.push({
              kind: "issue",
          ...
          for (const issue of topLevel) {
            const parent = issue.parent_id
              ? issues.find((i) => i.id === issue.parent_id)
              : null;
        ...
        <div className="ml-auto flex items-center gap-3">
          ...
          <div className="relative">
            ...
            {showSortMenu && (
              <div
                ref={sortMenuRef}
                className="absolute right-0 top-full mt-1 z-50 min-w-[140px] bg-[var(--color-bg-elevated)] border border-[var(--color-border-default)] rounded-[var(--radius-md)] shadow-[var(--shadow-popover)] py-1"
              >
              ...
              >
                <div
                  ...
                  className={`
                    ...
                    ${!isDropGroup && isFocused && keyboardNav ? "bg-[var(--color-bg-hover)] ring-1 ring-inset ring-[var(--color-accent-primary)]/40" : !isDropGroup && isFocused ? "bg-[var(--color-bg-hover)]" : !isDropGroup ? "bg-[var(--color-bg-secondary)]" : ""}
                  `
                    .trim()
                    .replace(/\s+/g, " ")}
                >
                    ...
                    <div key={issue.id} className="relative">
                      ...
                      >
                        {hasChildren ? (
                          ...
                          <span
                            className={`w-4 shrink-0 flex items-center justify-center text-[var(--color-border-default)] ${canDrag ? "cursor-grab active:cursor-grabbing" : ""}`}
                          >
                            <svg
                              width="12"
                              height="16"
                          ...
                          <span
                            className={`text-[var(--color-text-muted)] transition-opacity w-4 shrink-0 flex items-center justify-center ${canDrag ? "opacity-30 cursor-grab active:cursor-grabbing" : "opacity-0 group-hover:opacity-30"}`}
                          >
                            <svg
                              width="6"
                              height="10"
                              ...
                              <Popover onClose={() => setOpenPopover(null)}>
                                ...
                                {STATUS_OPTIONS.map((opt) => (
                                  <button
                                    key={opt.value}
                                    onClick={() =>
                                      handleQuickStatus(issue.id, opt.value)
                                    }
                                    className={`flex items-center gap-2 w-full px-3 py-1.5 text-sm transition-colors hover:bg-[var(--color-bg-hover)] ${opt.value === issue.status ? "text-[var(--color-accent-primary)]" : "text-[var(--color-text-primary)]"}`}
                                  >
                        ...
                        {parentBreadcrumb && (
                          <span className="text-sm text-[var(--color-text-muted)] truncate shrink-0 max-w-[150px]">
                            {parentBreadcrumb}
                          </span>
                        )}
                        {parentBreadcrumb && (
                        ...
                        {issue.priority > 0 && (
                          <span
                            className={`text-xs font-medium shrink-0 ${issue.priority === 1 ? "text-[var(--color-error)]" : issue.priority === 2 ? "text-[var(--color-warning)]" : "text-[var(--color-text-muted)]"}`}
                          >
                            {issue.priority === 1
                              ? "!!!"
                              : issue.priority === 2
                            ...
                            onClick={() =>
                              setOpenPopover(
                                openPopover?.issueId === issue.id &&
                                  ...
                                  ? null
                                  : { issueId: issue.id, type: "estimate" },
                              )
                            }
                              ...
                              <Popover onClose={() => setOpenPopover(null)}>
                                ...
                                {ESTIMATE_OPTIONS.map((est) => (
                                  <button
                                    ...
                                    className={`flex items-center gap-2 w-full px-3 py-1.5 text-sm transition-colors hover:bg-[var(--color-bg-hover)] ${est === (issue.estimate || 0) ? "text-[var(--color-accent-primary)]" : "text-[var(--color-text-primary)]"}`}
                                  >
                                    <span>
                                      {est === 0
                                        ? "No estimate"
  ...
  return (
    <svg width="14" height="14" viewBox="0 0 16 16" className="shrink-0">
      ...
      <circle
        ...
        fill="none"
        stroke={
          pct === 100 ? "var(--color-success)" : "var(--color-accent-primary)"
        }
        strokeWidth="2"
        strokeLinecap="round"
